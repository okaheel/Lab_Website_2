...
function loadStatsPage(){
	...
	var winner = 0;
	var loser = 0;
	for(var i = 2; i < statTable.rows.length;i++){
		if(parseInt(statTable.rows[i].cells[2].innerHTML) > parseInt(statTable.rows[i].cells[3].innerHTML)){
			statTable.rows[i].cells[4].innerHTML = "CU Boulder";
			winner = winner + 1;
		}
		else{
			statTable.rows[i].cells[4].innerHTML = statTable.rows[i].cells[1].innerHTML;
			loser = loser + 1;
		}
	}
...
function loadPlayersPage(){
	/*var players = [{player_name: 'Player 1'}, {player_name: 'Player 2'}, {player_name: 'Player 3'}, {player_name: 'Player 4'}];*/

	var tags = "";
	for(var i = 0; i<players.length;i++){
		tags += '<a class="dropdown-item" href=# onclick="switchPlayers(' +i+')">' +players[i].name+ '</a>';
	}
	document.getElementById('player_selector').innerHTML = tags;
}

function switchPlayers(playerNum){
	/*document.getElementById("p_year").innerHTML = players[i];
	document.getElementById("p_major").innerHTML = players[i];*/
